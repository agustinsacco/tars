...
| -------- | ------------------------------------- | ------------------------------------ |
| `create` | `title`, `prompt`, `schedule`, `mode` | Add an enabled task                  |
...
`schedule` accepts a five-field cron expression or an ISO date/time. `mode` is required for create.
Use `notify`, `on-failure`, `on-change`, `action-required`, `digest`, or `silent` to make delivery
intent explicit.
