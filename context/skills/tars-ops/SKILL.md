...
Tars stores its configuration in `~/.tars/config.json` (auto-generated by `tars setup`) and sensitive secrets (Discord tokens, API keys) in `~/.tars/.env`. Use the `secret` command to safely modify environment-level settings.
