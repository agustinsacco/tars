...
export class GeminiEngine extends EventEmitter {
    ...
    public async compressSession(force: boolean = false): Promise<boolean> {
        ...
        try {
                ...
                if (history && history.length > 20) {
                    ...
                    if (cutIndex < history.length) {
                        ...
                        try {
                            ...
                            summaryContent = response?.candidates?.[0]?.content?.parts?.[0]?.text || '';
                        } catch (err: any) {
                            logger.warn(`Semantic compression inference failed: ${err.message}`);
                        }

                        if (!summaryContent) {
                            summaryContent = "*(Summary generation failed, falling back to raw truncation)*";
                        }
                        ...
                        const newHistory = [
                            ...
                            {
                                role: 'model',
                                parts: [{ text: 'Got it. I will keep this historical context in mind.' }]
                            },
                            ...tail
                        ];
            ...
            return result.compressionStatus === 'compressed';
        } catch (e: any) {
