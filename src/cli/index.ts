...
program.command('start').description('Start the assistant supervisor in the background').action(start);

program.command('stop').description('Stop the assistant supervisor').action(stop);

program.command('restart').description('Check for updates and restart the assistant').action(restart);

program.command('status').description('Check the status of the assistant supervisor').action(status);
...
program.command('logs').description('View real-time logs from the assistant supervisor').action(logs);
...
program.command('uninstall').description('Uninstall the assistant and remove all data').action(uninstall);
