...
export async function quota() {
    ...
    try {
        ...
        if (!quotaResult || !quotaResult.buckets) {
            ...
            console.log(
                chalk.dim(
                    '1. Current auth method does not support quota tracking (e.g. legacy auth).'
                )
            );
            console.log(chalk.dim('2. Code Assist server is not reachable.'));
            console.log(
                chalk.dim('3. You are not using a Google Cloud project for authentication.\n')
            );
            return;
        }
        ...
        const relevantBuckets = buckets.filter(
            (b: any) => b.modelId && b.modelId.includes('gemini')
        );
