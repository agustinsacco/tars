...
1. **Structured vs. Unstructured Tasks**: OpenClaw's `HEARTBEAT.md` is easier for a human to edit. 
   - *Strategy*: We should add a Tars command `tars brain` to easily edit the steering manifest.
2. **Citation Support**: OpenClaw shows source lines for memories.
   - *Strategy*: Tars' `KnowledgeStore` has `startLine` support; we should expose this in the AI's search tool output.
